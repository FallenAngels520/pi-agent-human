...
export class TaskBoard {
	...
	scanUnclaimed(domain: string): Task[] {
		...
		return all.filter(
			(t) => t.status === "pending" && t.owner === "" && t.blockedBy.every((bid) => completed.has(bid)),
		);
	}
	 ...
	 */
	claim(domain: string, taskId: number, learnerName: string): { ok: true; task: Task } | { ok: false; error: string } {
