...
export class HeartbeatRunner {
	...
	private tick(): void {
		...
		try {
			...
			const blindSpots = concepts.filter((c) => c.confidence < this.config.blindSpotThreshold);

			if (blindSpots.length > 0) {
				const topics = blindSpots.map((c) => c.name);
				pool.submit(`heartbeat-${new Date().toISOString().slice(0, 10)}`, topics);
			}
		} catch {
...
export class CronService {
	...
	private execute(job: CronJobDef, _now: Date): void {
		...
		if (payload.kind === "agent_turn" && payload.message) {
			...
			if (concepts.length > 0) {
				this.config.pool.submit(`cron-${job.id}`, concepts);
			}
		}
