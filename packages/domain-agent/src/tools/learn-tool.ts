...
export class BackgroundPool {
  ...
  submit(
    ...
    const runOneLearner = (name: string) => {
      ...
      const learner = this.createSingleLearner(domain, name);
  ...
  private async runLearningRound(
    ...
    //
    // For now, this function signals that the Learner has claimed the task
    // and will proceed through its agent loop. The calling code (tick())
    // expects this to be async and calls update/retry after completion.
    //
...
const learnTopicSchema = Type.Object({
  ...
  learner_count: Type.Optional(
    Type.Number({ description: "Number of parallel learners (default: 2)" }),
  ),
});
...
export function createCheckLearningTool(
  pools: Map<string, BackgroundPool>,
): AgentTool<typeof checkLearningSchema> {
  return {
    name: "check_learning",
