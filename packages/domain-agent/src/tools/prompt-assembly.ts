...
import type { Task, TaskBoard } from "../task-board.ts";
...
export function loadBootstrap(workspaceDir: string): BootstrapFiles {
	const read = (name: string): string => {
		try {
			return readFileSync(`${workspaceDir}/${name}`, "utf-8").trim();
		} catch {
			return "";
		}
	};
...
function capabilityProfile(kg: KnowledgeGraph): string {
	...
	if (byDomain.size > 0) {
		lines.push("Known domains:", ...[...byDomain].map(([d, n]) => `  - ${d}: ${n} concepts`));
	}
	return layer("Capability", lines.join("\n"));
}

/** Build learning status summary from TaskBoard. */
function _learningStatus(taskBoard?: TaskBoard): string {
	if (!taskBoard) return "";
...
export function buildMainSystemPrompt(context: MainPromptContext = {}): string {
	...
	// L1: Identity
	const identity = bfs?.identity || "You are a coding agent with a self-evolving knowledge system.";
	...
	if (context.activeLearning && context.activeLearning.length > 0) {
		const lines = context.activeLearning.map((l) => `  - ${l.domain}: ${l.completed}/${l.total} tasks done`);
...
export function buildLearnerSystemPrompt(context: LearnerPromptContext): string {
	...
	// L1: Focused Identity
	const taskDesc = context.task ? `task #${context.task.id}: "${context.task.subject}"` : context.domain;
	sections.push(`You are a learning specialist. Your ONLY job: learn about ${taskDesc}.`);
