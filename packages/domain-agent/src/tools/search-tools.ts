...
const webSearchSchema = Type.Object({
	...
	allowed_domains: Type.Optional(
		Type.Array(Type.String(), {
			description: "Only include search results from these domains. Maps to Tavily include_domains (max 300).",
		}),
	),
	blocked_domains: Type.Optional(
		Type.Array(Type.String(), {
			description: "Never include search results from these domains. Maps to Tavily exclude_domains (max 150).",
		}),
	),
	...
	search_depth: Type.Optional(
		Type.String({
			description: "Search depth: basic (fast, 1 credit), advanced (higher relevance, 2 credits), fast, ultra-fast",
		}),
	),
	topic: Type.Optional(
		Type.String({
	...
	include_answer: Type.Optional(
		Type.Boolean({
			description: "Include an LLM-generated answer summarizing the search results",
		}),
	),
});
		...
		execute: async (
			...
			try {
				...
				if (!response.ok) {
					...
					else if (response.status === 432) errorDetail = "Monthly quota exceeded (1000 requests)";
 ...
 */
export type RunPromptFn = (content: string, prompt: string, signal?: AbortSignal) => Promise<string>;
...
export function createWebFetchTool(runPrompt?: RunPromptFn): AgentTool<typeof webFetchSchema> {
	return {
		...
		execute: async (
			...
			// ── HTTP → HTTPS upgrade ──
			const { url } = upgradeToHttps(params.url);
			...
			if (markdown) {
				...
				try {
					...
					if (response.redirected && isCrossHostRedirect(url, response.url)) {
						return {
							content: [
								{
									type: "text",
					...
					if (signal?.aborted) {
						return {
							content: [{ type: "text", text: `Fetch aborted for ${url}` }],
